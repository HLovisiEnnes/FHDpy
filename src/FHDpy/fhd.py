...
------------------------------------------------------------------------------
Implements main FHDLong class, which computes an intersection sequence
presentation for the Heegaard diagram of a 3-manifold.
------------------------------------------------------------------------------
Utils:
    FHDLong
    FHD_genus1
...
class FHDLong:
    ...
    def __init__(self,
        ...
        for edge in generators_standard.keys():
            self.maps[edge.lower()], self.maps[edge.upper()] = self.make_maps(
                edge, self.generators[edge])
        ...
        for curve in self.generators_edge.keys():
            ...
            inverse_edges[curve.upper()] = [edge.upper()
                                            for edge in self.generators_edge[curve]]
    ...
    def make_maps(self, edge: str, slp: SLP.SLP) -> tuple[SLP.SLP, SLP.SLP]:
        ...
        map_slp = SLP.SLP(
            slp.list_form + [edge, '#' + str(len(slp.list_form)-1) + '.#' + str(len(slp.list_form))])
    ...
    def dehn_twist(self, word: str, compress: bool = True):
        ...
        # list(word) is a list where each entry is a character of word.
        ...
        if any(char.isdigit() for char in split_word):
            ...
            while i < len(split_word):
                if split_word[i] != '^':  # If not a power.
                    ...
                    # Skip will tell how much of the next characters are numbers.
                    skip = 0

                elif split_word[i].isdigit():
                    # Delete the last entry of the clean word as
                    letter_part = clean_split_word.pop()
                    # it will be updated to contain the digits part.
                    digit = split_word[i]
                    skip = 0
                    while split_word[i + skip].isdigit():
                        # Add next digit to the curve.
                        digit += split_word[i + skip]
                        skip += 1
                        if (i + skip) >= len(split_word):
                            break  # If we finish the word, break.

                    # Add curve to the list of clean words.
                    clean_split_word.append(letter_part + digit)
                    ...
                    while split_word[i + 1 + skip].isdigit():
                        # Add next digit to the power.
                        power += split_word[i + 1 + skip]
                        skip += 1
                        if (i + 1 + skip) >= len(split_word):
                            break  # If we finish the word, break.

                    # Add power of curve to the list of clean words.
                    clean_split_word.append(raised + power)
                    ...
                    power_edges = []
                    # Get all edges whose of the raisd Dehn twist.
                    for edge in self.generators_edge[raised]:
                        self.maps[edge + power] = (
                            ...
                            self.make_maps(
                                edge.lower(),
                                self.generators[edge.lower()].get_power(
                                    int(power[1:]))
                            )[1]
                        )
    ...
    def fundamental_group(self) -> FPG.FinitePresentation:
        ...
        # Which edges are in alphas.
        edges_to_substitute = set(dictionary_edges_to_alpha.keys())
        # Delete everything that is not in a alpha.
        edges_to_delete = self.edges - edges_to_substitute
        ...
        for beta in self.beta:
            ...
            for edge in edges_to_substitute:
                beta_copy.substitute(
                    edge, dictionary_edges_to_alpha[edge], inplace=True)
...
def get_compressed(word: str) -> str:
    ...
    while i < len(word):
        char = word[i]
        # If the current character equals the last character or its inverse.
        if char.lower() == last_char.lower():
            skip = 1
            # Avoid querying a value outsiede the length of the string.
            if skip + i >= len(word):
                skip = 1
            else:
            ...
            # Subword in which only the character or its inverses occur.
            subword = last_char + word[i: i + skip]

            # Count the number of occurences of the positive minus negative of the symbol.
            signed_occurences = subword.count(
                last_char.lower()) - subword.count(last_char.upper())
            ...
            # If the net occurences of the symbol in the subword is +1 or -1,
            elif abs(signed_occurences) == 1:
